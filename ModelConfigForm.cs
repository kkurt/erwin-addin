...
namespace EliteSoft.Erwin.AddIn
{
    ...
    public partial class ModelConfigForm : Form
    {
        ...
        private void BtnAlterWizardProd_Click(object sender, EventArgs e)
        {
            ...
            // Inform the user about the upcoming automated UI operations.
            var resp = MessageBox.Show(this,
                "Alter Script will be generated by driving erwin's Complete Compare wizard\n" +
                "and the Forward Engineer Alter Script wizard automatically.\n\n" +
                "You will see dialogs open and close on screen. Please do NOT interact\n" +
                "with erwin during this process (about 10 seconds).\n\n" +
                "Continue?",
                "Generate DDL", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
            if (resp != DialogResult.OK) return;

            btnAlterWizardProd.Enabled = false;
            lblDDLStatus.Text = "Generating DDL via erwin wizard...";
            lblDDLStatus.ForeColor = Color.Gray;
            rtbDDLOutput.Text = "";

            // Small top-left corner wait dialog, TopMost=true so the user sees
            // progress in the foreground throughout the automation. Small size +
            // top-left placement keeps it from overlapping erwin's centered CC /
            // RD / Alter Script dialogs. WM_COMMAND and mouse_event targeting
            // do not depend on Z-order, so TopMost is safe here.
            Form waitDialog = new Form
            {
                Size = new System.Drawing.Size(380, 120),
                FormBorderStyle = FormBorderStyle.None,
                StartPosition = FormStartPosition.Manual,
                Location = new Point(30, 120),   // below erwin's main-frame title bar
                TopMost = true,
                ShowInTaskbar = false,
                BackColor = Color.White
            };
            // Amber title bar - distinct from erwin's blue title band so the two don't blend.
            var titlePanel = new Panel { Dock = DockStyle.Top, Height = 30, BackColor = Color.FromArgb(230, 126, 34) };
            titlePanel.Controls.Add(new Label {
                Text = "Generating DDL", ForeColor = Color.White,
                Font = new Font("Segoe UI", 9.5f, FontStyle.Bold),
                Location = new Point(10, 5), AutoSize = true
            });
            waitDialog.Controls.Add(titlePanel);
            waitDialog.Controls.Add(new Label {
                Text = "Driving erwin's compare + alter-script wizards.\nPlease wait...",
                Dock = DockStyle.Fill,
                TextAlign = System.Drawing.ContentAlignment.MiddleCenter,
                Font = new System.Drawing.Font("Segoe UI", 10f)
            });
            waitDialog.Paint += (s2, e2) => {
                using (var pen = new Pen(Color.FromArgb(230, 126, 34), 1))
            ...
            else if (rbFromDB.Checked)
            {
                ...
                // has a real Right-side model to compare against. Skip this if
                // the connection isn't configured yet.
                if (string.IsNullOrEmpty(_dbConnectionString) || string.IsNullOrEmpty(_dbLabel))
                {
                    try { waitDialog.Close(); waitDialog.Dispose(); } catch { }
                    lblDDLStatus.Text = "Error: click 'Configure...' to set DB connection first.";
                ...
                {
                    try { waitDialog.Close(); waitDialog.Dispose(); } catch { }
                    lblDDLStatus.Text = "Error: click 'Select Tables...' and pick at least one table.";
                    lblDDLStatus.ForeColor = Color.Red;
                    btnAlterWizardProd.Enabled = true;
                    return;
                }
                ...
                {
                    Log("");
                    Log("-- Reverse-engineering DB into new PU (silent)...");
                    Action<string> logCb = Log;
                    dynamic rePU = Services.DdlGenerationService.ReverseEngineerToSession(
                        _scapi, _currentModel,
                        _dbHost, _dbName, _dbUser, _dbPassword, _dbUseWindowsAuth,
                        _dbTypeCode, _dbTargetServer, _dbTargetVersion,
                        _dbSchema, _dbSelectedTables,
                        logCb);
                    string reName = rePU?.Name?.ToString() ?? "";
                    Log($"-- RE done. New PU = '{reName}'");
                    cfg.Right = Services.RightModelSource.OpenModel;
                    cfg.OpenModelName = reName;
                    rePUToCleanup = rePU;   // remember to close after alter-script capture
                }
                catch (Exception ex)
                {
                    try { waitDialog.Close(); waitDialog.Dispose(); } catch { }
                    lblDDLStatus.Text = $"RE error: {ex.Message}";
                    lblDDLStatus.ForeColor = Color.Red;
                    btnAlterWizardProd.Enabled = true;
                    return;
                }
            ...
            var svc = new Services.WizardAutomationService(msg => {
                if (InvokeRequired) BeginInvoke(new Action(() => Log(msg)));
                else Log(msg);
            });

            dynamic scapiLocal = _scapi;
            dynamic rePULocal = rePUToCleanup;
            System.Threading.Tasks.Task.Run(() =>
            {
                string script = null;
                string err = null;
                try { script = svc.CaptureAlterScript(cfg, timeoutMs: 60000); }
                catch (Exception ex) { err = $"{ex.GetType().Name}: {ex.Message}"; }

                // Close the RE'd PU (if any) so it doesn't linger in the session.
                if (rePULocal != null)
                {
                    try
                    {
                        scapiLocal.PersistenceUnits.Remove(rePULocal, false);
                        BeginInvoke(new Action(() => Log("-- RE'd PU removed from session")));
                    }
                    catch (Exception ex)
                    {
                        BeginInvoke(new Action(() => Log($"-- RE'd PU remove failed: {ex.Message}")));
                    }
                }
                ...
                BeginInvoke(new Action(() =>
                {
                    ...
                    else if (string.IsNullOrEmpty(script))
                    {
                        ...
                        rtbDDLOutput.Text = "-- No differences between models.\n";
                    }
                    else
                    {
                    ...
                    // Restore focus to the add-in form after driving erwin's wizards
                    this.TopMost = true;
                    this.Activate();
                    this.BringToFront();
                    this.TopMost = false;
        ...
        // === Live SCAPI Monitor: high-frequency PropertyBag / Session / MessageLog diff.
        // Not RE-specific — also captures Compare, Alter Script, Forward Engineer, Save, etc. ===
        ...
        private List<System.IO.FileSystemWatcher> _liveMonFsWatchers = new List<System.IO.FileSystemWatcher>();
        private string _liveMonLastClipboard = null;
        ...
        /// <summary>
        /// Keyword highlight for PropertyBag values / MessageLog entries: flags strings
        /// that hint at Compare / Alter / Forward / Merge / Schema / Generate operations.
        /// </summary>
        private static string MonHighlight(string text)
        {
            if (string.IsNullOrEmpty(text)) return text;
            string[] kw = { "Compare", "Alter", "Forward", "Merge", "Schema", "Generate", "Synch", "Version" };
            foreach (var k in kw)
            {
        ...
        private void BtnLiveReMon_Click(object sender, EventArgs e)
        {
            if (_liveMonRunning)
            {
                ...
                foreach (var w in _liveMonFsWatchers)
                {
                    try { w.EnableRaisingEvents = false; w.Dispose(); } catch { }
                }
                _liveMonFsWatchers.Clear();
                _liveMonRunning = false;
                btnLiveReMon.Text = "Live Monitor";
                btnLiveReMon.BackColor = System.Drawing.Color.White;
            ...
            Log("");
            Log("========================================");
            Log("=== LIVE SCAPI MONITOR STARTED (100ms) ===");
            Log("========================================");
            Log("Captures PU / Session / PropertyBag / MessageLog / ModelDirectories + FS + Clipboard.");
            Log("Run any erwin action (Compare, Alter Script, RE, Forward, Save). Press button again to STOP.");
            Log("");

            // File system watchers — catch any file erwin writes during the operation.
            var roots = new[]
            {
            ...
            foreach (var root in roots)
            {
                try
                {
                    ...
                    var fsw = new System.IO.FileSystemWatcher(root)
                    {
                        IncludeSubdirectories = true,
                        NotifyFilter = System.IO.NotifyFilters.FileName | System.IO.NotifyFilters.LastWrite | System.IO.NotifyFilters.Size,
                        InternalBufferSize = 64 * 1024,
                    };
                    fsw.Created += (s, ev) => { try { Log(MonHighlight($"[FS] NEW: {ev.FullPath}")); } catch { } };
                    fsw.Changed += (s, ev) =>
                    {
                        try
                        {
                            // Filter noisy frequent writes — only log SQL/DDL/XML/ERWIN/ALTER/SCRIPT/FET ext or names.
                            string ext = System.IO.Path.GetExtension(ev.FullPath).ToLowerInvariant();
                            string name = System.IO.Path.GetFileName(ev.FullPath);
                            bool interesting =
                                ext == ".sql" || ext == ".ddl" || ext == ".xml" || ext == ".erwin" ||
                                ext == ".erx" || ext == ".fet" || ext == ".xls" || ext == ".xlsx" ||
                                name.IndexOf("alter", StringComparison.OrdinalIgnoreCase) >= 0 ||
                                name.IndexOf("compare", StringComparison.OrdinalIgnoreCase) >= 0 ||
                                name.IndexOf("script", StringComparison.OrdinalIgnoreCase) >= 0 ||
                                name.IndexOf("schema", StringComparison.OrdinalIgnoreCase) >= 0 ||
                                name.IndexOf("forward", StringComparison.OrdinalIgnoreCase) >= 0;
                            if (interesting) Log(MonHighlight($"[FS] CHG: {ev.FullPath}"));
                        }
                        catch { }
                    };
                    fsw.EnableRaisingEvents = true;
                    _liveMonFsWatchers.Add(fsw);
                    Log($"[FS] Watching: {root}");
                }
                catch (Exception fex) { Log($"[FS] could not watch {root}: {fex.Message}"); }
            }
        ...
        private void LiveMonTick()
        {
            ...
            try
            {
                ...
                // ----- Clipboard poll every 10 ticks (~1s) -----
                if (_liveMonTickNo % 10 == 0)
                {
                    try
                    {
                        if (Clipboard.ContainsText())
                        {
                            ...
                            if (!string.Equals(clip, _liveMonLastClipboard, StringComparison.Ordinal))
                            {
                                int previewLen = Math.Min(clip?.Length ?? 0, 200);
                                string preview = (clip ?? "").Substring(0, previewLen).Replace("\n", " ").Replace("\r", "");
                                Log(MonHighlight($"[{t}ms] CLIPBOARD changed: len={clip?.Length ?? 0}, preview: {preview}"));
                                _liveMonLastClipboard = clip;
                            }
                        }
                    }
                    catch (Exception ex) { MonLogOnce("Clipboard.poll", ex); }
                }
                ...
                for (int i = 0; i < puCount; i++)
                {
                    ...
                    else
                    {
                        foreach (var kv in current)
                        {
                            ...
                            {
                                string v = kv.Value; if (v.Length > 220) v = v.Substring(0, 220) + "...";
                                Log(MonHighlight($"[{t}ms] +{label}.{kv.Key} = {v}"));
                            }
                            else if (oldVal != kv.Value)
                            {
                                ...
                                Log(MonHighlight($"[{t}ms] *{label}.{kv.Key}: '{oV}' -> '{nV}'"));
                            }
                        }
                ...
                if (msgBag != null)
                {
                    ...
                    // Aggressive: log on ANY length change (up OR down — erwin may reset
                    // MessageLog between operations) AND log is-empty→not-empty transitions.
                    bool msgChanged = arrLen != _liveMonLastMsgLogCount;
                    if (msgChanged)
                    {
                        if (_liveMonLastMsgLogCount == -1)
                            Log($"[{t}ms] MessageLog INITIAL: Is_Empty={isEmpty}, Log.Length={arrLen}");
                        else if (arrLen < _liveMonLastMsgLogCount)
                            Log($"[{t}ms] *** MessageLog RESET: {_liveMonLastMsgLogCount} -> {arrLen} (operation boundary?)");
                        else
                            Log($"[{t}ms] *** MessageLog growth: len {_liveMonLastMsgLogCount} -> {arrLen} (Is_Empty={isEmpty})");

                        if (logArr is Array arr && arr.Length > 0)
                        {
                            // On growth, log only new entries; on reset/initial, log full array.
                            int start = (arrLen > _liveMonLastMsgLogCount && _liveMonLastMsgLogCount > 0)
                                ? _liveMonLastMsgLogCount : 0;
                            for (int mi = start; mi < arr.Length; mi++)
                            {
                                try { Log(MonHighlight($"  MSG[{mi}]: {arr.GetValue(mi)}")); }
        ...
        /// <summary>
        /// DIAGNOSTIC: comprehensive sweep trying to trigger targeted-diff alter DDL from FEModel_DDL.
        /// Variants tested:
        ///   (A) OptionXML with &lt;Option name="AlterStatements" value="1"/&gt; child
        ///   (B) OptionXML with CompareLevel="FE Alter Schema Report"
        ///   (C) OptionXML wrapped in &lt;ErwinOptionSet Version="1"&gt;...&lt;/ErwinOptionSet&gt;
        ///   (D) OptionXML with SourceModelPath attribute + multiple &lt;Option&gt; children
        ///   (E) Set DLSourceModelPath on PU PropertyBag before FEModel_DDL
        /// </summary>
        private void BtnTestAlterFE_Click(object sender, EventArgs e)
        {
            btnTestAlterFE.Enabled = false;
            string baseOptXml = @"c:\temp\alter_schema_gen.xml";
            try
            {
                Log("");
                Log("========================================================");
                Log("=== TEST: FEModel_DDL Alter-mode comprehensive sweep ===");
                Log("========================================================");
                if (_currentModel == null) { Log("No active model."); return; }
                if (!System.IO.File.Exists(baseOptXml))
                {
                    Log($"Option XML missing: {baseOptXml}");
                    return;
                }

                // Baseline .erwin to use in SourceModelPath-style tests
                string baselinePath = null;
                using (var dlg = new OpenFileDialog
                {
                    Title = "Baseline .erwin dosyasi sec (opsiyonel - Cancel=skip)",
                    Filter = "erwin model (*.erwin)|*.erwin",
                    CheckFileExists = true
                })
                {
                    if (dlg.ShowDialog(this) == DialogResult.OK) baselinePath = dlg.FileName;
                    else Log("(No baseline selected - skipping source-path variants)");
                }

                string baseXmlContent = System.IO.File.ReadAllText(baseOptXml);
                string escapedBaseline = baselinePath != null
                    ? System.Security.SecurityElement.Escape(baselinePath) : null;

                // (0) Baseline - no changes - for reference
                RunVariant("0: baseline (unchanged XML)", baseXmlContent);

                // (A) Add <Option name="AlterStatements" value="1"/> child before first <Node>
                string xmlA = InjectOptionChild(baseXmlContent,
                    "<Option name=\"AlterStatements\" value=\"1\"/>");
                RunVariant("A: <Option AlterStatements=1>", xmlA);

                // (A2) Multiple likely option names
                string xmlA2 = InjectOptionChild(baseXmlContent,
                    "<Option name=\"AlterStatements\" value=\"1\"/>" +
                    "<Option name=\"IsAlterScriptGeneration\" value=\"1\"/>" +
                    "<Option name=\"MergeAlterStatements\" value=\"1\"/>" +
                    "<Option name=\"PKConstraintInAlter\" value=\"1\"/>" +
                    "<Option name=\"FKConstraintInAlter\" value=\"1\"/>" +
                    "<Option name=\"ColumnConstraintInAlter\" value=\"1\"/>" +
                    "<Option name=\"TableConstraintInAlter\" value=\"1\"/>");
                RunVariant("A2: all Alter* option children", xmlA2);

                // (B) Change CompareLevel to alter-specific values
                foreach (var level in new[] {
                    "FE Alter Schema Report",
                    "FE Alter Report",
                    "FE Alter",
                    "Alter Schema Generation"
                })
                {
                    string xmlB = System.Text.RegularExpressions.Regex.Replace(baseXmlContent,
                        "CompareLevel=\"[^\"]*\"", $"CompareLevel=\"{level}\"");
                    RunVariant($"B: CompareLevel='{level}'", xmlB);
                }

                // (C) Wrap in <ErwinOptionSet>
                string xmlC = $"<ErwinOptionSet Version=\"1\">{baseXmlContent}</ErwinOptionSet>";
                RunVariant("C: wrapped in <ErwinOptionSet>", xmlC);

                // (D) Combined: SourceModelPath attribute + Option children
                if (escapedBaseline != null)
                {
                    var rootMatch = System.Text.RegularExpressions.Regex.Match(baseXmlContent, @"<TreeState\b([^>]*)>");
                    if (rootMatch.Success)
                    {
                        string xmlD = baseXmlContent.Replace(rootMatch.Value,
                            $"<TreeState DLSourceModelPath=\"{escapedBaseline}\"{rootMatch.Groups[1].Value}>" +
                            "<Option name=\"AlterStatements\" value=\"1\"/>" +
                            "<Option name=\"IsAlterScriptGeneration\" value=\"1\"/>");
                        RunVariant("D: DLSourceModelPath attr + AlterStatements option", xmlD);
                    }
                }

                // (E) Set DLSourceModelPath on PU PropertyBag before calling
                if (escapedBaseline != null)
                {
                    try
                    {
                        Log("");
                        Log("--- Variant: E: set DLSourceModelPath on PU PropertyBag ---");
                        dynamic propBag = _currentModel.PropertyBag(Type.Missing, Type.Missing);
                        try { propBag.Value["DLSourceModelPath"] = baselinePath; Log("  set DLSourceModelPath OK"); }
                        catch (Exception ex) { Log($"  set DLSourceModelPath failed: {ex.Message}"); }
                        try { propBag.Value["FESourceModelPath"] = baselinePath; Log("  set FESourceModelPath OK"); }
                        catch (Exception ex) { Log($"  set FESourceModelPath failed: {ex.Message}"); }
                        try { _currentModel.PropertyBag(Type.Missing, Type.Missing, propBag); Log("  PropertyBag SET OK"); }
                        catch (Exception ex) { Log($"  PropertyBag SET failed: {ex.Message}"); }
                        RunVariant("E: after PropertyBag set", baseXmlContent);
                    }
                    catch (Exception ex) { Log($"  variant E error: {ex.Message}"); }
                }

                Log("");
                Log("=== Target if any variant works: ALTER>0, CREATE=0, DROP=0 ===");
            }
            catch (Exception ex) { Log($"TEST outer error: {ex.GetType().Name}: {ex.Message}"); }
            finally { btnTestAlterFE.Enabled = true; }
        }

        /// <summary>Injects option-child XML just before the first &lt;Node&gt; element inside &lt;TreeState&gt;.</summary>
        private static string InjectOptionChild(string xml, string optionFragment)
        {
            int nodeIdx = xml.IndexOf("<Node", StringComparison.Ordinal);
            if (nodeIdx < 0) return xml;
            return xml.Substring(0, nodeIdx) + optionFragment + xml.Substring(nodeIdx);
        }

        private void RunVariant(string label, string xmlContent)
        {
            string tmpXml = System.IO.Path.Combine(System.IO.Path.GetTempPath(),
                $"alter_variant_{Guid.NewGuid():N}.xml");
            string outSql = System.IO.Path.Combine(System.IO.Path.GetTempPath(),
                $"alter_out_{Guid.NewGuid():N}.sql");
            try
            {
                System.IO.File.WriteAllText(tmpXml, xmlContent);
                Log("");
                Log($"--- Variant: {label} ---");
                long t0 = Environment.TickCount64;
                object result = null;
                try { result = _currentModel.FEModel_DDL(outSql, tmpXml); }
                catch (Exception ex) { Log($"  THREW: {ex.GetType().Name}: {ex.Message}"); return; }
                long elapsed = Environment.TickCount64 - t0;

                if (!System.IO.File.Exists(outSql)) { Log($"  returned {result}, no output file"); return; }
                long sz = new System.IO.FileInfo(outSql).Length;
                string content = System.IO.File.ReadAllText(outSql);
                int alter = System.Text.RegularExpressions.Regex.Matches(content, @"\bALTER\s+TABLE\b",
                    System.Text.RegularExpressions.RegexOptions.IgnoreCase).Count;
                int create = System.Text.RegularExpressions.Regex.Matches(content, @"\bCREATE\s+TABLE\b",
                    System.Text.RegularExpressions.RegexOptions.IgnoreCase).Count;
                int drop = System.Text.RegularExpressions.Regex.Matches(content, @"\bDROP\s+TABLE\b",
                    System.Text.RegularExpressions.RegexOptions.IgnoreCase).Count;
                Log($"  returned {result} in {elapsed}ms, {sz}B, ALTER={alter} CREATE={create} DROP={drop}");
                if (alter > 0 && create == 0 && drop == 0)
                    Log($"  *** TARGETED ALTER DETECTED *** variant: {label}");
            }
            finally
            {
                try { System.IO.File.Delete(tmpXml); } catch { }
                try { System.IO.File.Delete(outSql); } catch { }
            }
        }

        /// <summary>
        /// DIAGNOSTIC: dump every PU's PropertyBag to a timestamped file.
        /// Uses explicit COM late-binding via InvokeMember because the dynamic
        /// dispatcher does not resolve ISCPropertyBag.Key/Value.
        /// SCAPI PersistenceUnits.Item is 0-indexed (Count items at 0..Count-1).
        /// </summary>
        private void BtnDumpPropBag_Click(object sender, EventArgs e)
        {
            btnDumpPropBag.Enabled = false;
            string stamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
            string dumpPath = System.IO.Path.Combine(@"c:\tmp", $"propbag_{stamp}.log");
            try { System.IO.Directory.CreateDirectory(@"c:\tmp"); } catch { }
            var sb = new System.Text.StringBuilder();
            sb.AppendLine("================================================================");
            sb.AppendLine($"PropertyBag dump @ {DateTime.Now:yyyy-MM-dd HH:mm:ss.fff}");
            sb.AppendLine("================================================================");

            // Hybrid: dynamic for COM iteration (handles VARIANT marshalling),
            // InvokeMember only for members the DLR can't find (PropertyBag.Name/Value)
            static object InvokeMethod(object target, string name, params object[] args)
            {
                try
                {
            ...
            {
                Log("");
                Log($"=== Dump PropBag -> {dumpPath} ===");
                if (_scapi == null) { Log("_scapi is null"); return; }

                dynamic pus = _scapi.PersistenceUnits;
                int count = (int)pus.Count;
                sb.AppendLine($"PU count: {count}");
                Log($"  PU count: {count}");

                // SCAPI.Item accepts multiple index bases across contexts - try robustly
                // We probe indices 0..count inclusive; failures just get skipped
                for (int idx = 0; idx <= count; idx++)
                {
                    ...
                    sb.AppendLine();
                    sb.AppendLine($"--- PU[{idx}] ---");

                    foreach (var p in new[] { "Name", "IsValid", "HasSession" })
                    {
                        try
                        {
                            object v = pu.GetType().InvokeMember(p,
                                System.Reflection.BindingFlags.GetProperty | System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public,
                                null, pu, null);
                            sb.AppendLine($"  {p} = {v}");
                        }
                        catch (Exception ex) { sb.AppendLine($"  [ERR] {p}: {ex.Message.Split('\n')[0]}"); }
                    }

                    // ISCPropertyBag's Name(idx)/Value(idx) enumeration isn't exposed via IDispatch
                    // on this PU kind - DISP_E_MEMBERNOTFOUND. Workaround: probe known keys by
                    // name, using PropertyBag(filter) + Value(keyName) pattern.
                    string[] candidates = new[]
                    {
                        // Documented
                        "Locator", "Active Model", "Model_Type",
                        "Target_Server", "Target_Server_Version", "Target_Server_Minor_Version",
                        // From DLL string scans (CC/DL/FE engine state keys)
                        "DLSourceModelPath", "FESourceModelPath", "CCSourceModelPath",
                        "CCTargetModelPath", "RemoteModelSavePath", "ModelSavePath",
                        "DL_Active", "DL_OptionSet", "DL_ScriptOutputPath",
                        "DL_FEOptionSetPath", "DL_CompareLevel", "DL_Script_Active",
                        "DL_DBBrandName", "DL_DBVersion", "DL_ModelType", "DLOutputPath", "DLOptionSetPath",
                        "CC_Active", "CC_CompareLevel", "CC_OptionSet",
                        "CC_MartFolderActive", "CC_Mart_SavingLatestVersion", "CC_Mart_UsingLatestVersion",
                        // Alter-specific guesses
                        "AlterStatements", "IsAlterScriptGeneration", "MergeAlterStatements",
                        "IsAlterScript", "AlterScript_Active", "ActionType",
                        // Common persistence bag keys
                        "ReadOnly", "Save_Version", "Save_Path", "Save_Date",
                        "Model_Name", "FullName", "Path"
                    };

                    // Try multiple access strategies; Value is DISPID 0 (default property),
                    // not a named member, so InvokeMember("Value",...) fails with MEMBERNOTFOUND.
                    // Default-member syntax: InvokeMember("", GetProperty | InvokeMethod) with the
                    // index as args - this hits DISPID 0 via OleAut32's late binding.
                    object ReadDefault(object target, object idx)
                    {
                        var flags = System.Reflection.BindingFlags.GetProperty
                                  | System.Reflection.BindingFlags.InvokeMethod;
                        return target.GetType().InvokeMember("", flags, null, target, new[] { idx });
                    }
                    ...
                    {
                        object rawPb = null;
                        try { rawPb = pu.PropertyBag(key, true); }
                        catch { }
                        if (rawPb == null) { notFound++; continue; }

                        int fc = -1;
                        try { fc = Convert.ToInt32(((dynamic)rawPb).Count); } catch { }

                        // Filter-based bag: if the key doesn't exist, bag may have Count=0
                        if (fc == 0) { notFound++; continue; }

                        // Read value via DEFAULT property (DISPID 0) - VB's propBag.Value(0)
                        object val = null;
                        string readMethod = "";
                        try { val = ReadDefault(rawPb, 0); readMethod = "default(0)"; }
                        catch (Exception ex1)
                        {
                            try { val = ReadDefault(rawPb, key); readMethod = "default(name)"; }
                            catch
                            {
                                try { val = InvokeMethod(rawPb, "Value", 0); readMethod = "Value(0)"; }
                                catch
                                {
                                    readMethod = $"ALL FAILED: d0={ex1.Message.Split('\n')[0]}";
                                }
                            }
                        }
                        ...
                        if (val is Array arr)
                        {
                            ...
                            foreach (var elem in arr) parts.Add(elem?.ToString() ?? "<null>");
                            vStr = $"[array:{parts.Count}] " + string.Join("|", parts);
                        }
                        else vStr = val == null ? "<null>" : val.ToString();

                        sb.AppendLine($"    {key} = {vStr}  (bagCount={fc}, via={readMethod})");
                        found++;
                    }
                    sb.AppendLine($"  Probed {candidates.Length} keys: found={found}, notFound={notFound}");

                    // Also dump unfiltered bag contents - try DEFAULT property iteration
                    // to discover the 11 entries (5 we know by name, 6 unknown).
                    try
                    {
                        dynamic pbAll = pu.PropertyBag();
                        int allCount = (int)pbAll.Count;
                        sb.AppendLine($"  --- Unfiltered bag ({allCount} entries) ---");
                        for (int i = 0; i < allCount; i++)
                        {
                            object val = null;
                            string err = null;
                            try { val = ReadDefault(pbAll, i); }
                            catch (Exception ex) { err = ex.Message.Split('\n')[0]; }

                            string vStr;
                            if (err != null) vStr = $"[ERR: {err}]";
                            else if (val == null) vStr = "<null>";
                            else if (val is Array arr2)
                            {
                                var parts = new System.Collections.Generic.List<string>();
                                foreach (var elem in arr2) parts.Add(elem?.ToString() ?? "<null>");
                                vStr = $"[array:{parts.Count}] " + string.Join("|", parts);
                            }
                            else vStr = val.ToString();
                            // Truncate huge values
                            if (vStr.Length > 200) vStr = vStr.Substring(0, 197) + "...";
                            sb.AppendLine($"    [{i}] {vStr}");
                        }
                    }
                    catch (Exception ex) { sb.AppendLine($"  unfiltered bag error: {ex.Message.Split('\n')[0]}"); }
                }
            }
            catch (Exception ex) { sb.AppendLine($"[OUTER ERR] {ex.GetType().Name}: {ex.Message}"); }
            finally
            {
                try
                {
                    System.IO.File.WriteAllText(dumpPath, sb.ToString());
                    Log($"  wrote {new System.IO.FileInfo(dumpPath).Length} bytes");
                }
                catch (Exception ex) { Log($"  write failed: {ex.Message}"); }
                btnDumpPropBag.Enabled = true;
            }
        }
        ...
        /// <summary>
        /// DIAGNOSTIC: walk the UI Automation tree of erwin's main window and
        /// dump it to a file. Used to plan the CC -> Right Alter Script wizard
        /// automation path. Answers: are XTP Ribbon controls visible to UIA?
        /// </summary>
        private void BtnDumpUiaTree_Click(object sender, EventArgs e)
        {
            btnDumpUiaTree.Enabled = false;
            string stamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
            string path = System.IO.Path.Combine(@"c:\tmp", $"uia_tree_{stamp}.log");
            try { System.IO.Directory.CreateDirectory(@"c:\tmp"); } catch { }
            var sb = new System.Text.StringBuilder();
            sb.AppendLine("================================================================");
            sb.AppendLine($"UIA tree dump @ {DateTime.Now:yyyy-MM-dd HH:mm:ss.fff}");
            sb.AppendLine("================================================================");
            try
            {
                Log("");
                Log($"=== Dump UIA Tree -> {path} ===");

                uint myPid = (uint)System.Diagnostics.Process.GetCurrentProcess().Id;
                sb.AppendLine($"Our process PID: {myPid}");

                // Enumerate ALL top-level windows belonging to erwin.exe (our process)
                var allHwnds = new System.Collections.Generic.List<(IntPtr Hwnd, string Title, string Class, bool Visible)>();
                EnumWindows((hwnd, lp) =>
                {
                    ...
                    allHwnds.Add((hwnd, title.ToString(), cls.ToString(), vis));
                    return true;
                }, IntPtr.Zero);

                sb.AppendLine("");
                sb.AppendLine($"=== All top-level windows of erwin.exe (PID {myPid}) ===");
                foreach (var w in allHwnds)
                    sb.AppendLine($"  HWND 0x{w.Hwnd.ToInt64():X}  visible={w.Visible}  class='{w.Class}'  title='{w.Title}'");

                // Find erwin main window: likely class starts with "Afx" (MFC) and title contains "erwin"
                IntPtr hMain = IntPtr.Zero;
                string mainDesc = null;
                foreach (var w in allHwnds)
                {
                    if (!w.Visible) continue;
                    bool isMain = (w.Title != null && w.Title.IndexOf("erwin", StringComparison.OrdinalIgnoreCase) >= 0)
                               || (w.Class != null && (w.Class.StartsWith("Afx") || w.Class.Contains("XTP") || w.Class.StartsWith("ERwin")));
                    if (isMain && !w.Title.Contains("Elite Soft"))
                    {
                        hMain = w.Hwnd;
                        mainDesc = $"0x{w.Hwnd.ToInt64():X} class='{w.Class}' title='{w.Title}'";
                        break;
                    }
                }
                if (hMain == IntPtr.Zero)
                {
                    // Fallback: any visible window that isn't our Elite Soft dialog
                    foreach (var w in allHwnds)
                    {
                        if (w.Visible && !w.Title.Contains("Elite Soft") && !string.IsNullOrEmpty(w.Title))
                        {
                            hMain = w.Hwnd;
                            mainDesc = $"0x{w.Hwnd.ToInt64():X} class='{w.Class}' title='{w.Title}' (fallback)";
                            break;
                        }
                    }
                }
                if (hMain == IntPtr.Zero) { Log("  erwin main window not found"); sb.AppendLine("[ERR] no suitable window"); return; }
                sb.AppendLine($"");
                sb.AppendLine($"Selected erwin main window: {mainDesc}");

                var rootElem = System.Windows.Automation.AutomationElement.FromHandle(hMain);
                if (rootElem == null) { Log("  UIA could not get element from HWND"); sb.AppendLine("[ERR] AutomationElement.FromHandle returned null"); return; }

                sb.AppendLine("");
                sb.AppendLine("=== UIA tree (depth-limited to 6, max 500 nodes) ===");
                int totalNodes = 0;
                int maxNodes = 500;
                WalkUiaLimited(rootElem, 0, 6, sb, ref totalNodes, maxNodes);
                sb.AppendLine("");
                sb.AppendLine($"Total nodes visited: {totalNodes} (cap {maxNodes})");

                sb.AppendLine("");
                sb.AppendLine("=== Keyword search across entire tree ===");
                string[] keywords = { "Complete Compare", "Alter Script", "Schema Generation",
                    "Forward Engineer", "Actions", "Derive Model", "Compare Wizard",
                    "Save Script", "Ribbon" };
                foreach (var kw in keywords)
                {
                    try
                    {
                        var cond = new System.Windows.Automation.PropertyCondition(
                            System.Windows.Automation.AutomationElement.NameProperty, kw);
                        var found = rootElem.FindAll(System.Windows.Automation.TreeScope.Descendants, cond);
                        int hits = found.Count;
                        sb.AppendLine($"  '{kw}' -> {hits} hit(s)");
                        for (int i = 0; i < Math.Min(hits, 5); i++)
                        {
                            var elem = found[i];
                            sb.AppendLine($"    [{i}] ControlType={elem.Current.ControlType?.ProgrammaticName}  Name='{elem.Current.Name}'  AutomationId='{elem.Current.AutomationId}'");
                        }
                    }
                    catch (Exception ex) { sb.AppendLine($"  '{kw}' -> search error: {ex.Message.Split('\n')[0]}"); }
                }

                // Also search for partial matches using "contains" logic
                sb.AppendLine("");
                sb.AppendLine("=== Partial-name search (descendants, all elements scanned) ===");
                try
                {
                    var all = rootElem.FindAll(System.Windows.Automation.TreeScope.Descendants,
                        System.Windows.Automation.Condition.TrueCondition);
                    sb.AppendLine($"  Total descendants: {all.Count}");
                    var wantKw = new[] { "compare", "alter", "forward", "schema", "ribbon", "generate", "derive", "action" };
                    foreach (var kw in wantKw)
                    {
                        int hits = 0;
                        for (int i = 0; i < all.Count && hits < 20; i++)
                        {
                            try
                            {
                                string nm = all[i].Current.Name ?? "";
                                if (nm.IndexOf(kw, StringComparison.OrdinalIgnoreCase) >= 0)
                                {
                                    sb.AppendLine($"    [{kw}] {all[i].Current.ControlType?.ProgrammaticName} Name='{nm}' Id='{all[i].Current.AutomationId}'");
                                    hits++;
                                }
                            }
                            catch { }
                ...
                catch (Exception ex) { sb.AppendLine($"  descendant scan error: {ex.Message.Split('\n')[0]}"); }
            }
            catch (Exception ex) { sb.AppendLine($"[OUTER ERR] {ex.GetType().Name}: {ex.Message}"); Log($"  error: {ex.Message}"); }
            finally
            {
                try
                {
                    // Strip invalid surrogate pairs, write UTF-8 (no BOM)
                    string content = SanitizeForFile(sb.ToString());
                    System.IO.File.WriteAllText(path, content, new System.Text.UTF8Encoding(false));
                    Log($"  wrote {new System.IO.FileInfo(path).Length} bytes");
                }
                catch (Exception ex) { Log($"  write failed: {ex.Message}"); }
                btnDumpUiaTree.Enabled = true;
            }
        }
        ...
        private static string SanitizeForFile(string s)
        {
            ...
            var sb2 = new System.Text.StringBuilder(s.Length);
            for (int i = 0; i < s.Length; i++)
            {
                char c = s[i];
                if (char.IsHighSurrogate(c))
                {
                    if (i + 1 < s.Length && char.IsLowSurrogate(s[i + 1])) { sb2.Append(c); sb2.Append(s[i + 1]); i++; }
                    else sb2.Append('?'); // lone high surrogate
                }
                else if (char.IsLowSurrogate(c)) sb2.Append('?'); // lone low surrogate
                else sb2.Append(c);
            }
            return sb2.ToString();
        }

        private static void WalkUiaLimited(System.Windows.Automation.AutomationElement elem, int depth, int maxDepth,
            System.Text.StringBuilder sb, ref int totalNodes, int maxNodes)
        {
            if (elem == null || depth > maxDepth || totalNodes >= maxNodes) return;
            totalNodes++;
            string indent = new string(' ', depth * 2);
            string ct = "?", name = "", id = "", cls = "";
            try { ct = elem.Current.ControlType?.ProgrammaticName ?? "?"; } catch { }
            try { name = elem.Current.Name ?? ""; } catch { }
            try { id = elem.Current.AutomationId ?? ""; } catch { }
            try { cls = elem.Current.ClassName ?? ""; } catch { }
            if (name.Length > 80) name = name.Substring(0, 77) + "...";
            sb.AppendLine($"{indent}{ct} name='{name}' id='{id}' class='{cls}'");

            try
            {
                var children = elem.FindAll(System.Windows.Automation.TreeScope.Children,
                    System.Windows.Automation.Condition.TrueCondition);
                foreach (System.Windows.Automation.AutomationElement child in children)
                {
                    if (totalNodes >= maxNodes) break;
                    WalkUiaLimited(child, depth + 1, maxDepth, sb, ref totalNodes, maxNodes);
                }
            }
            catch { }
        }
        ...
        /// <summary>
        /// 0-CLICK AUTOMATION: drives erwin's Complete Compare + Alter Script Schema
        /// Generation Wizard via WM_COMMAND + UIA, extracts the alter DDL text,
        /// and cancels all dialogs. Runs on a background thread to avoid blocking UI.
        /// </summary>
        private void BtnAlterWizard_Click(object sender, EventArgs e)
        {
            btnAlterWizard.Enabled = false;
            Log("");
            Log("================================================================");
            Log("=== 0-CLICK Alter Script via erwin's own wizard ===");
            Log("================================================================");

            var svc = new Services.WizardAutomationService(msg => {
                // Marshal log writes back to UI thread
                if (InvokeRequired) BeginInvoke(new Action(() => Log(msg)));
                else Log(msg);
            });

            // Run on background thread because the automation posts messages + waits,
            // and we must NOT block the main message pump (erwin's WndProc needs to
            // process our WM_COMMAND + open its dialogs).
            System.Threading.Tasks.Task.Run(() =>
            {
                string script = null;
                string err = null;
                try { script = svc.CaptureAlterScript(timeoutMs: 60000); }
                catch (Exception ex) { err = $"{ex.GetType().Name}: {ex.Message}"; }
                ...
                BeginInvoke(new Action(() =>
                {
                    ...
                    {
                        Log("!!! Extracted empty script");
                    }
                    else
                    {
                        string outPath = System.IO.Path.Combine(@"c:\tmp", $"alter_wizard_{DateTime.Now:yyyyMMdd_HHmmss}.sql");
                        try { System.IO.Directory.CreateDirectory(@"c:\tmp"); } catch { }
                        try { System.IO.File.WriteAllText(outPath, script); Log($"Saved: {outPath} ({script.Length} chars)"); }
                        catch (Exception ex) { Log($"Save failed: {ex.Message}"); }

                        Log("");
                        Log("--- Script preview (first 30 lines) ---");
                        var lines = script.Split('\n');
                        for (int i = 0; i < Math.Min(lines.Length, 30); i++)
                            if (!string.IsNullOrWhiteSpace(lines[i])) Log($"  | {lines[i].TrimEnd()}");
                        if (lines.Length > 30) Log($"  | ... (+{lines.Length - 30} more)");
                    }
                    btnAlterWizard.Enabled = true;
                }));
            });
        }
