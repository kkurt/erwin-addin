...
///                  --model=lib/name
///
/// NOTE: SCAPI's ReverseEngineer cannot be called out-of-process — it silently no-ops
/// when not running inside the live erwin instance. The "From DB" diff path is therefore
/// implemented IN-PROCESS (in the add-in's own SCAPI handle), not here.
/// </summary>
class Program
{
    [System.STAThread]
    static int Main(string[] args)
    {
        ...
        {
            Console.Error.WriteLine("Usage: DdlHelper --action=ddl|versions ...");
            return 1;
        }
