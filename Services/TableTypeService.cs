...
namespace EliteSoft.Erwin.AddIn.Services
{
    ...
    public class TableTypeService
    {
        ...
        /// <summary>
        /// Load table types from database using DatabaseService configuration
        /// </summary>
        public bool LoadTableTypes()
        {
            try
            {
                ...
                if (!DatabaseService.Instance.IsConfigured)
                {
                    ...
                    _isLoaded = false;
                    System.Diagnostics.Debug.WriteLine($"TableTypeService: {_lastError}");
                    return false;
                }

                string dbType = DatabaseService.Instance.GetDbType();
                string query = GetTableTypeQuery(dbType);
        ...
        /// </summary>
        private string GetTableTypeQuery(string dbType)
        {
            switch (dbType?.ToUpper())
            {
                case "POSTGRESQL":
                    return "SELECT \"ID\", \"NAME\", \"AFFIX\", \"NAME_EXTENSION_LOCATION\" FROM \"TABLE_TYPE\"";
                ...
                case "MSSQL":
                default:
                    return "SELECT [ID], [NAME], [AFFIX], [NAME_EXTENSION_LOCATION] FROM [dbo].[TABLE_TYPE]";
            }
        }
